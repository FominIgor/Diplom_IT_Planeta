...
class AvailableDocuments:
    def __init__(self, table_widget, db_manager, teacher_id):
        ...
        self.user_id = teacher_id  # <-- добавьте эту строку
    ...
    def update_table(self):
        ...
        try:
            ...
            query = """
                ...
                WHERE d.Teacher = %s AND es.Status != %s
                ORDER BY d.id DESC
            """


            documents = self.db_manager.execute_query(query, (self.user_id, "Сохранен"))
            ...
            for row_idx, doc in enumerate(documents):
                ...
                for col in range(self.table_widget.columnCount()):
                    ...
                    status = doc.get('Execution_Status', '').strip().lower()

                    if status == 'принят завкафедрой':
    ...
    def modify_record(self):
        ...
        try:
            ...
            dialog = QDialog()
            dialog.setWindowTitle(f"Загрузка документов для ID: {record_id}")
            layout = QVBoxLayout()
    ...
    def _upload_file(self, file_type):
        """Загрузка файла и автообновление статуса"""
        try:
            path, _ = QFileDialog.getOpenFileName(None, f"Выберите файл ({file_type})", "", "Все файлы (*)")
            if not path:
                return
            ...
            if file_type == "with_stamp":
                self.set_status_by_name("Сохранен")
            elif file_type == "without_printing":
                # Если файл с печатью не загружен, установить "На проверке"
                with_stamp = self.file_data.get("with_stamp")
                if not with_stamp or isinstance(with_stamp, str):
                    self.set_status_by_name("На проверке")

            QMessageBox.information(None, "Успешно", f"Файл успешно загружен:\n{original_name}")

        except Exception as e:
            logging.error(f"Ошибка загрузки: {traceback.format_exc()}")
            QMessageBox.critical(None, "Ошибка загрузки", f"Не удалось загрузить файл:\n{str(e)}")

    def set_status_by_name(self, status_name):
        """Устанавливает статус в ComboBox по названию"""
        try:
            query = "SELECT id FROM Execution_Status WHERE Status = %s"
            result = self.db_manager.execute_query(query, (status_name,))
            if result:
                status_id = result[0]['id']
                index = self.status_combo.findData(status_id)
                if index != -1:
                    self.status_combo.setCurrentIndex(index)
                    logging.info(f"Статус '{status_name}' (id={status_id}) установлен автоматически")
                else:
                    logging.warning(f"Статус '{status_name}' найден в БД, но отсутствует в ComboBox")
            else:
                logging.warning(f"Статус '{status_name}' не найден в Execution_Status")
        except Exception as e:
            logging.error(f"Ошибка установки статуса '{status_name}': {e}")
    ...
    def _download_file(self, doc_id, file_type):
        ...
        try:
            ...
            path, _ = QFileDialog.getSaveFileName(None, "Сохранить файл", filename, "Все файлы (*)")
            if path:
