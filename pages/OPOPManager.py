...
class OPOPManager:
    ...
    def update_table(self):
        try:
            query = "SELECT id, OPOP_path, Syllabus_path, Program_path, Speciality FROM OPOP_UP_Program ORDER BY id LIMIT 100;"
    ...
    def upload_record(self):
        ...
        try:
            ...
            program_button = QPushButton("Загрузить Шаблон")
    ...
    def save_record(self, dialog):
        ...
        try:
            ...
            query = """
                INSERT INTO OPOP_UP_Program (OPOP_path, Syllabus_path, Program_path, Speciality, OPOP_ext, Syllabus_ext, Program_ext)
            ...
            dialog.accept()
            self.update_table()  # Обновляем таблицу после сохранения записи
        except Exception as e:
            logging.error(f"Ошибка при сохранении записи: {e}")
            QMessageBox.critical(self.table_widget, "Ошибка", f"Не удалось сохранить запись: {e}")
    ...
    def download_file(self, row, column):
        ...
        try:
            ...
            query = f"SELECT {column_name} FROM OPOP_UP_Program WHERE id = %s"
            result = self.db_manager.execute_query(query, (record_id,))

            if result and result[0][column_name]:
                linux_path = result[0][column_name]
                
                # Convert Linux path to Windows path (if running on Windows)
                import platform
                if platform.system() == "Windows":
                    # Replace Linux path with a valid Windows path (e.g., "C:/uploads/...")
                    # Alternatively, map `/home/user/uploads/` to a local directory
                    windows_path = linux_path.replace("/home/user/uploads/", "C:/uploads/").replace("/", "\\")
                else:
                    windows_path = linux_path  # Keep original path on Linux
                ...
                if save_path:
                    # Copy the file (if accessible)
                    import shutil
                    try:
                        shutil.copy2(windows_path, save_path)
                        QMessageBox.information(self.table_widget, "Успех", f"Файл успешно скачан: {save_path}")
                    except FileNotFoundError:
                        QMessageBox.critical(self.table_widget, "Ошибка", f"Файл не найден: {windows_path}")
            else:
