...
from pages.DisciplineManager import DisciplineManager
from pages.DocumentDistributor import DocumentDistributor
from pages.DocumentChecker import DocumentChecker
...
class WelcomeScreen(QDialog):
    ...
    def count_available_documents(self):
        """
        Считает количество документов, которые еще нужно выполнить
        (все кроме тех, у которых статус 'Сохранен').
        """
        if not self.db_manager or not hasattr(self, 'current_user_id'):
            return 0
        ...
        try:
            query = """
                ...
                WHERE d.Teacher = %s
                AND es.Status IS NOT NULL
                AND LOWER(es.Status) != LOWER(%s)
            """
            result = self.db_manager.execute_query(query, (self.current_user_id, "сохранен"))
            return result[0]['doc_count'] if result else 0
        except Exception as e:
            logging.error(f"Ошибка при подсчёте документов: {e}")
            return 0
    ...
    def AddDiscipline_button(self):
        ...
        self.table_widget = self.findChild(QtWidgets.QTableWidget, 'tableWidget')
        self.document_viewer = DisciplineManager(self.table_widget, self.db_manager, self.current_user_id)
    ...
    def DistributeDocuments_button(self):
        ...
        self.table_widget = self.findChild(QtWidgets.QTableWidget, 'tableWidget')
        self.document_viewer = DocumentDistributor(self.table_widget, self.db_manager, self.current_user_id)

    def count_documents_to_distribute(self):
        """
        Считает количество документов, которые действительно подлежат распределению —
        только те, которые соответствуют фильтру, используемому в DocumentDistributor.
        """
        if not self.db_manager or not hasattr(self, 'current_user_id'):
            return 0
...
SELECT COUNT(*) AS doc_count
FROM Documents d
JOIN Discipline dis ON d.Discipline = dis.id
JOIN Speciality spec ON dis.Speciality = spec.id
JOIN Department dep ON spec.Department = dep.id
WHERE d.Teacher IS NULL
  AND dep.Head_of_the_department = %s

            """
            result = self.db_manager.execute_query(query, (self.current_user_id,))
            return result[0]['doc_count'] if result else 0
        except Exception as e:
            logging.error(f"Ошибка при подсчете документов на распределение: {e}")
            return 0


    def update_documents_to_distribute_count(self):
        """
        Обновляет текст кнопок распределения документов с числом необработанных.
        """
        doc_count = self.count_documents_to_distribute()
        self.DistributeDocuments.setText(f"Распределить документы ({doc_count})")
        self.DistributeDocuments1.setText(f"({doc_count})")
    ...
    def CheckDocuments_button(self):
        ...
        self.table_widget = self.findChild(QtWidgets.QTableWidget, 'tableWidget')
        self.document_viewer = DocumentChecker(self.table_widget, self.db_manager, self.current_user_id)
    ...
    def handle_user_access(self, user_id, full_name, roles):
        ...
        # Обновить текст кнопки AvailableDocuments с количеством документов
        doc_count = self.count_available_documents()
        self.AvailableDocuments.setText(f"Назначенные документы ({doc_count})")
        self.AvailableDocuments1.setText(f"({doc_count})")
        self.update_available_documents_count()
        # Обновить текст кнопки DistributeDocuments
        self.update_documents_to_distribute_count()



    def update_available_documents_count(self):
        doc_count = self.count_available_documents()
        self.AvailableDocuments.setText(f"Назначенные документы ({doc_count})")
        self.AvailableDocuments1.setText(f"({doc_count})")
