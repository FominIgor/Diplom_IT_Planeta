...
class SpecialityManager:
   ...
   def update_table(self):
        """Обновляет данные в таблице с исправлениями"""
        try:
            ...
            for row in rows:
                ...
                # Убедимся, что используем правильные ключи из row
                self.table_widget.setItem(row_position, 0, QTableWidgetItem(str(row.get('id', ''))))
                self.table_widget.setItem(row_position, 1, QTableWidgetItem(str(row.get('Name', ''))))
                self.table_widget.setItem(row_position, 2, QTableWidgetItem(str(row.get('Department', ''))))
            
            # Восстанавливаем сортировку
            self.table_widget.setSortingEnabled(True)
            if sort_column >= 0:
                self.table_widget.sortByColumn(sort_column, sort_order)
                
        except Exception as e:
            logging.error(f"Ошибка при обновлении таблицы: {e}")
            QMessageBox.critical(self.table_widget, "Ошибка", f"Не удалось обновить данные: {str(e)}")